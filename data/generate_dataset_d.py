"""
ArthX Synthetic Seed Data Generator — Dataset D: MOSTLY CLEAN
==============================================================
Same schema as A, but:
  - Only 4 injected anomalies (3 genuine, 1 high-impact borderline)
  - Stable, slightly positive net cash flow throughout — no crisis trend
  - Mild seasonality (utility/payroll variance) to keep data realistic
  - Cash flow trend is flat-ish with mild seasonal oscillation
  - Proves the system DOES NOT over-flag when data is clean
  - Useful for demonstrating discrimination ability of the ML model

SEED = 419 (deterministic)
"""
...
DATA_DIR = Path(__file__).parent / "dataset_d"
DATA_DIR.mkdir(exist_ok=True)

START_DATE = date(2025, 10, 1)
END_DATE   = date(2026, 9, 25)
HIGH_IMPACT_ANOMALY_DATE = date(2026, 7, 20)
...
VENDORS = [
    ...
    {"name": "ClientFirst Revenue",   "category": "Revenue",               "type": "inflow",  "pattern": "monthly_first", "mean": 105000.00,"std": 7000.00,  "invoice_fraction": 0.0},  # Higher revenue — stays net positive
    {"name": "ProjectAlpha Revenue",  "category": "Revenue",               "type": "inflow",  "pattern": "irregular",     "mean": 22000.00, "std": 5000.00,  "invoice_fraction": 0.0},
    {"name": "RetainerB2B Inc",       "category": "Revenue",               "type": "inflow",  "pattern": "monthly_15th",  "mean": 15000.00, "std": 500.00,   "invoice_fraction": 0.0},
...
def seasonal_multiplier(d, vendor):
    if not vendor.get("seasonal", False):
        return 1.0
    month = d.month
    if month in (1, 12):   return random.uniform(1.15, 1.28)
    if month in (6, 7, 8): return random.uniform(1.08, 1.20)
    return random.uniform(0.88, 1.02)

def mild_trend_multiplier(d):
    """Almost flat — very mild natural drift, no designed crisis."""
    trend_start = END_DATE - timedelta(days=60)
    if d < trend_start:
        return 1.0
    days_into_trend = (d - trend_start).days
    # Only 8% increase at most — well within normal variance
    increase = 0.08 * (days_into_trend / 60.0)
    return 1.0 + increase
...
# ---------------------------------------------------------------------------
# Generate normal transactions — stable cash flow
# ---------------------------------------------------------------------------
transactions = []
tx_id = 1
    ...
    for vendor in VENDORS:
        ...
        for tx_date in dates:
            ...
            if vendor["type"] == "outflow":
                amount *= mild_trend_multiplier(tx_date)
...
# ---------------------------------------------------------------------------
# Inject VERY FEW anomalies (3 genuine + 1 high-impact)
# ---------------------------------------------------------------------------
anomaly_manifest = []
...
few_anomaly_specs = [
    ("DataSync SaaS",       date(2026, 2, 1),  28500.00, "AMOUNT_OUTLIER",         "Amount is 5.9x vendor mean ($4,800) — only clear anomaly in SaaS"),
    ("ArunCapital Payroll", date(2026, 6, 12), 52400.00, "OFF_SCHEDULE",           "Payroll paid mid-month (expected 1st or 15th)"),
    ("DataSync SaaS",       date(2026, 5, 1),  4850.00,  "DUPLICATE_TRANSACTION",  "Duplicate SaaS payment May 1"),
    ("DataSync SaaS",       date(2026, 5, 2),  4850.00,  "DUPLICATE_TRANSACTION",  "Duplicate SaaS payment May 2 (pair with May 1)"),
]

for vendor_name, tx_date, amount, anomaly_type, reason in few_anomaly_specs:
...
# HIGH-IMPACT anomaly — smaller, borderline, proves the system is calibrated
HIGH_IMPACT_TX_ID = tx_id
high_impact_amount = 32000.00
...
high_impact_tx = {
    ...
    "type": "outflow",
    "status": "normal",
    "description": "ANOMALY [HIGH-IMPACT]: Utility charge $32,000 — 10x vendor average. Still a real anomaly but in otherwise clean data.",
    "_is_injected": True, "_high_impact": True,
}
add_anomaly(
    high_impact_tx,
    f"Utility charge $32,000 vs vendor mean ${hi_stats['mean']:.0f} (z-score ~{hi_zscore:.0f}). "
    "Notable but isolated — overall data is clean.",
...
# ---------------------------------------------------------------------------
# Generate invoices — very few duplicates (3 pairs)
# ---------------------------------------------------------------------------
invoices = []
inv_id = 1
invoice_manifest = []
...
for tx in transactions:
    ...
    has_po = random.random() < 0.80  # Higher PO compliance in a clean dataset
    invoices.append({
        ...
        "status": random.choices(["paid", "pending", "disputed"], weights=[0.75, 0.22, 0.03])[0],
...
# ---------------------------------------------------------------------------
print("=" * 70)
print("DATASET D (MOSTLY CLEAN) — VERIFICATION")
print("=" * 70)
...
print("\n" + "=" * 70)
print("MONTHLY CASH FLOW TREND (should be stable/positive throughout)")
print("=" * 70)
monthly_flows = {}
for tx in transactions:
...
full_manifest = {
    "meta": {
        ...
        "description": "Mostly clean dataset: only 4 anomalies, stable positive cash flow with mild seasonality. Proves the model doesn't over-flag.",
    },
...
print("\nDone — Dataset D written to:", DATA_DIR)
